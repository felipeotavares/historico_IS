                    ...
                    if not linha_estacao.empty:
                        ...
                        if indice_hora:
                            ...
                            if fim_janela - inicio_janela + 1 >= 20:
                                ...
                                # Calculando derivada 2ª de H
                                dados_janela['ddH_nT'] = dados_janela['dH_nT'].diff 
                                # Extraindo amplitude 
                                amplitude = caract_amplitude(dados_janela)
                                
                                # Calculando a amplificação se a estação conjugada estiver disponível
                                conjugada = estacoes_conjugadas.get(estacao)
                                if conjugada:
                                    linha_conjugada = dados_por_estacao[dados_por_estacao['Estacao'] == conjugada]
                                    
                                    
                                    if not linha_conjugada.empty:
                                        dados_conjugada = linha_conjugada.iloc[0]['Dados']
                                        
                                        A = linha_conjugada['Dados']
                                        
                                        
                                        amplitude_conjugada = caract_amplitude(dados_conjugada)
                                        amplificacao = calcula_amplificacao(amplitude, '    ')
                                        display(conjugada)
                                        display(amplificacao)
                                        isplay(conjugada)
                                        display(amplitude_conjugada)
...
# # plot_amplificacao_por_data(lista_dados_agrupados_teste,2024)
# ano_selecionado = 2021
# plot_amplificacao_por_data(lista_dados_agrupados,ano_selecionado)
